...
import operators.fitness.fitness as fitness
import survivors.select_best as select_best
...
SURVIVORS_OPERATOR = SurvivorsStrategy.REPLACE_WORST
...
while current_generation < MAX_NUMBER_OF_GENERATIONS:
    ...
    population = SURVIVORS_OPERATOR(
        population,
        children,
        NUMBER_OF_INDIVIDUALS_TO_REPLACE,
        FITNESS_FUNCTION,
        cost_matrix
    )
    ...
    best_fitness_through_time.append(select_best.select_best(population, FITNESS_FUNCTION, cost_matrix))
...
def generate_output_file():
    ...
    with open(file_path, "w", encoding="UTF-8") as output_file:
        ...
        output_file.write(f"Selection operator: {SELECTION_OPERATOR.__name__.upper()}\n")
        if (SELECTION_OPERATOR.__name__.upper() == "TOURNAMENT"):
            output_file.write(f"Torunament size: {TOURNAMENT_SIZE}\n")
        output_file.write(f"Crossover operator: {CROSSOVER_OPERATOR.__name__.upper()}\n")
        output_file.write(f"Crossover probability: {CROSSOVER_PROBABILITY}\n")
        output_file.write(f"Mutation operator: {MUTATION_OPERATOR.__name__.upper()}\n")
        ...
        output_file.write(f"Survivors selection algorithm: {SURVIVORS_OPERATOR.__name__.upper()}\n")
